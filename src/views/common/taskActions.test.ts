...
describe("quickCaptureLineNl: быстрый ввод с NLP-датами", () => {
	...
	// setField правит ПОСЛЕДНИЙ существующий токен поля, поэтому NLP-дата молча
	// затирала бы набранную руками. Явное поле сильнее: NLP не применяется вовсе,
	// в т.ч. датное слово остаётся в тексте (иначе потерялось бы и оно).
	it("явное 📅 не затирается NLP-датой (и слово-дата остаётся в тексте)", () => {
		expect(quickCaptureLineNl("завтра позвонить 📅 2026-09-01", WED)).toBe(
			"- [ ] завтра позвонить 📅 2026-09-01",
		);
		expect(quickCaptureLineNl("позвонить 📅 2026-09-01 завтра в 15", WED)).toBe(
			"- [ ] позвонить 📅 2026-09-01 завтра в 15",
		);
	});

	it("другие эмодзи-поля NLP-дате не мешают", () => {
		expect(quickCaptureLineNl("завтра позвонить ⏳ 2026-09-01", WED)).toBe(
			"- [ ] позвонить ⏳ 2026-09-01 📅 2026-07-23",
		);
	});
});
...
describe("nlCaptureHint: живая подсказка распознанной даты", () => {
	...
	it("явное 📅 в тексте → null: подсказка не обещает НЕприменяемую дату", () => {
		expect(nlCaptureHint("завтра позвонить 📅 2026-09-01", WED)).toBeNull();
	});
});
