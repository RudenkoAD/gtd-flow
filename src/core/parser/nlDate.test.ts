...
describe("parseNlDate — календарные даты", () => {
	...
	// Десятичные числа, версии и номера пунктов стоят ровно там, где ищется
	// хвостовое выражение. Раньше «N.M» съедалось датой И ВЫРЕЗАЛОСЬ из текста —
	// пользователь молча терял набранное число (а дата уезжала в следующий год).
	it("однозначный месяц без года датой НЕ считается (версии, дроби, пункты)", () => {
		expect(parseNlDate("обновить до 1.2", WED)).toBeNull();
		expect(parseNlDate("прочитать главу 3.4", WED)).toBeNull();
		expect(parseNlDate("оплатить счёт 12.5", WED)).toBeNull();
		expect(parseNlDate("купить сахара 2.5", WED)).toBeNull();
	});

	it("двузначный месяц — по-прежнему дата (в т.ч. с однозначным днём)", () => {
		expect(parseNlDate("оплатить 5.08", WED)?.date).toBe("2026-08-05");
		expect(parseNlDate("1.09 линейка", WED)?.date).toBe("2026-09-01");
	});

	it("явный год снимает двусмысленность даже при однозначных день/месяц", () => {
		expect(parseNlDate("релиз 1.2.2027", WED)?.date).toBe("2027-02-01");
	});
});
