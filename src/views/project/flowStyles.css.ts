/**
 * CSS Svelte Flow (копия @xyflow/svelte@1.6.2 dist/style.css = base.css + тема).
 *
 * Почему строка, а не `import ".../style.css"`: esbuild с outfile main.js
 * выносит css-импорты в main.css, который Obsidian НЕ загружает (он грузит
 * только styles.css плагина), а esbuild.config.mjs — вне зоны этого модуля.
 * Инжекция в <head> через ensureFlowStyles(doc) заодно покрывает pop-out окна
 * (у них свой document — ТЗ §8: только el.win/el.doc, никаких глобалов).
 *
 * Файл сгенерирован скриптом из dist/style.css — не редактировать руками;
 * при обновлении @xyflow/svelte перегенерировать тем же способом.
 */

export const FLOW_STYLES: string = `/* this gets exported as style.css and can be used for the default theming */
...
const STYLE_EL_ID = "gtd-flow-xyflow-css";

/** Счётчик потребителей на document: последний размонтированный вид убирает <style>. */
const refCounts = new WeakMap<Document, number>();

/**
 * Инжектировать стили Svelte Flow в document (идемпотентно, с refcount).
 * Возвращает функцию освобождения — вызывать при размонтировании вида.
 */
export function ensureFlowStyles(doc: Document): () => void {
	const count = refCounts.get(doc) ?? 0;
	refCounts.set(doc, count + 1);
	if (count === 0 && doc.getElementById(STYLE_EL_ID) === null) {
		const el = doc.createElement("style");
		el.id = STYLE_EL_ID;
		el.textContent = FLOW_STYLES;
	...
	return () => {
		if (released) return;
		released = true;
		const cur = refCounts.get(doc) ?? 1;
		refCounts.set(doc, cur - 1);
		if (cur - 1 <= 0) doc.getElementById(STYLE_EL_ID)?.remove();
	};
}
