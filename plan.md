...
| Automated verification           | Verified on 2026-07-29                 | `npm run verify` passed on that date: lint, formatting, TypeScript, both Svelte checks, the unit/integration suite with coverage, five browser flows, production builds, packaged-plugin smoke, bundle budgets and release artifacts. `npm audit` reported zero vulnerabilities. Counts are in the snapshot below. |
...
Platform note: that snapshot was measured on the Linux runtime. On Windows the
same suite was red — the large-vault `FsVault` scan regression needed about 31 s
against the 30 s Vitest budget — so `npm run verify` did not pass on the
maintainer's machine. The benchmark now runs 3,000 notes by default and keeps the
original size available as `GTD_PERF_NOTES=10000`; before quoting a snapshot as
current, re-run the gate on the platform you are releasing from.
