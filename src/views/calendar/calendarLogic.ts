...
// ---------------------------------------------------------------------------
// Маркер «сейчас» в агенде (горизонтальная линия ● HH:mm ———, §сегодня)
// ---------------------------------------------------------------------------

/** Время начала ("HH:mm") элемента дня агенды/ячейки: у задачи — поле-размещение
 *  (placedTime), у события — время вхождения; null — без времени («весь день»). */
export function agendaItemTime(item: AgendaDayItem): string | null {
	return item.kind === "task" ? placedTime(item.ev.task, item.ev.field) : item.occ.time;
}

/**
 * Индекс вставки маркера «сейчас» (● HH:mm ———) в ОТСОРТИРОВАННЫЙ mergeDayItems
 * список элементов дня «сегодня»: маркер идёт ПОСЛЕ всех элементов, начавшихся
 * строго до nowMinutes, и ПЕРЕД первым с временем начала >= nowMinutes. Элементы
 * без времени / «весь день» всегда ВЫШЕ линии (mergeDayItems ставит их первыми —
 * здесь они считаются «до сейчас»). Список ДОЛЖЕН быть уже отсортирован
 * mergeDayItems — функция опирается на его порядок (без-времени, затем время asc).
 * Возвращает индекс в [0, items.length]: 0 — линия над всеми (пустой день тоже 0,
 * маркер один и виден), items.length — под всеми.
 */
export function nowMarkerIndex(items: readonly AgendaDayItem[], nowMinutes: number): number {
	let i = 0;
	for (const it of items) {
		const t = agendaItemTime(it);
		const min = t === null ? null : timeToMinutes(t);
		// без времени или начатое строго до «сейчас» — выше линии; на первом
		// элементе с временем >= «сейчас» останавливаемся (дальше всё позже — sort)
		if (min !== null && min >= nowMinutes) break;
		i += 1;
	}
	return i;
}
