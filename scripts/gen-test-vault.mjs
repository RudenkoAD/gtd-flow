 ...
 *
 * Использование:
 *   node scripts/gen-test-vault.mjs <папка-назначения> [--files N] [--tasks M] [--force]
 *
...
if (args.length === 0 || args[0].startsWith("--")) {
	console.error(
		"Использование: node scripts/gen-test-vault.mjs <папка-назначения> [--files N] [--tasks M] [--force]",
	);
...
const OWNERSHIP_MARKER = ".gtd-flow-test-vault";
const OWNERSHIP_MARKER_TEXT = "This directory was generated by scripts/gen-test-vault.mjs.";

if (targetDir === parse(targetDir).root) {
	console.error("Отказ: корень файловой системы нельзя использовать как тестовое хранилище.");
	process.exit(1);
}
...
/** Сначала строим полный план, затем одной предзаписью проверяем назначение. */
const plannedWrites = [];
function write(relPath, content) {
	plannedWrites.push({ relPath, content });
}
...
write(
	...
	[
		...
		"Files may be replaced only when the generator is run explicitly with --force.",
		"",
	].join("\n"),
);

// --- безопасная запись полного плана ---

const duplicatePaths = plannedWrites
	.map(({ relPath }) => relPath)
	.filter((relPath, index, all) => all.indexOf(relPath) !== index);
if (duplicatePaths.length > 0) {
	console.error(`Внутренняя ошибка: путь запланирован дважды: ${duplicatePaths.join(", ")}`);
	process.exit(1);
}

const targetExists = existsSync(targetDir);
if (targetExists && !statSync(targetDir).isDirectory()) {
	console.error(`Отказ: путь назначения существует и не является папкой: ${targetDir}`);
	process.exit(1);
}
if (targetExists && !force) {
	console.error(`Отказ: папка назначения уже существует: ${targetDir}`);
	console.error(
		"Выберите новую папку. --force разрешён только для ранее сгенерированного test vault.",
	);
	process.exit(1);
}

if (targetExists && force) {
	const markerPath = join(targetDir, OWNERSHIP_MARKER);
	let ownedByGenerator = false;
	try {
		ownedByGenerator =
			existsSync(markerPath) &&
			statSync(markerPath).isFile() &&
			readFileSync(markerPath, "utf8").startsWith(OWNERSHIP_MARKER_TEXT);
	} catch {
		// A raced, unreadable, or malformed marker is never proof that this is ours.
		ownedByGenerator = false;
	}
	if (!ownedByGenerator) {
		console.error(`Отказ: --force разрешён только для vault, отмеченного ${OWNERSHIP_MARKER}.`);
		console.error(
			"Маркер генератора отсутствует или невалиден; существующие файлы не изменены.",
		);
		process.exit(1);
	}
	const affected = plannedWrites
		.map(({ relPath }) => relPath)
		.filter((relPath) => existsSync(join(targetDir, relPath)));
	console.error(`ВНИМАНИЕ: включён --force для существующей папки: ${targetDir}`);
	if (affected.length === 0) {
		console.error("Существующие файлы не совпадают с планом; будут добавлены новые файлы:");
		for (const { relPath } of plannedWrites) console.error(`  + ${relPath}`);
	} else {
		console.error("Следующие существующие файлы будут перезаписаны:");
		for (const relPath of affected) console.error(`  ! ${relPath}`);
	}
}

mkdirSync(targetDir, { recursive: true });
for (const { relPath, content } of plannedWrites) {
	const abs = join(targetDir, relPath);
	mkdirSync(dirname(abs), { recursive: true });
	writeFileSync(abs, content, { encoding: "utf8", flag: force ? "w" : "wx" });
}
...
console.log(
	`  Bulk: ${FILES} файлов × ${TASKS} задач, доски: ${BOARDS.length}, проекты: ${PROJECTS.length}`,
);
